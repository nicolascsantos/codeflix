...
namespace FC.Codeflix.Catalog.IntegrationTests.Infra.Data.EF.Repositories.GenreRepository
{
    ...
    public class GenreRepositoryTest
    {
        ...
        public async Task Insert()
        {
            ...
            var categoriesExample = _fixture.GetExampleCategoriesList(3);
            categoriesExample.ForEach(category => exampleGenre.AddCategory(category.Id));
            await dbContext.Categories.AddRangeAsync(categoriesExample, CancellationToken.None);
            ...
            var genreCategoriesRelation = await assertsDbContext.GenresCategories
                .Where(r => r.GenreId == exampleGenre.Id)
                .ToListAsync();
            genreCategoriesRelation.Should().HaveCount(categoriesExample.Count);
            genreCategoriesRelation.ForEach(genreCategoriesRelation =>
            {
                var expectedCategory = categoriesExample.FirstOrDefault(x => x.Id == genreCategoriesRelation.CategoryId);
        ...
        public async Task Get()
        {
            ...
            await dbContext.Categories.AddRangeAsync(categoriesListExample, CancellationToken.None);
            await dbContext.Genres.AddAsync(exampleGenre, CancellationToken.None);
            foreach (var categoryId in exampleGenre.Categories)
            {
                var relation = new GenresCategories(categoryId, exampleGenre.Id);
                await dbContext.GenresCategories.AddAsync(relation, CancellationToken.None);
            }
            await dbContext.SaveChangesAsync(CancellationToken.None);
            ...
            genreFromRepository.Categories.Should().HaveCount(exampleGenre.Categories.Count);
            foreach (var cateogoryId in genreFromRepository.Categories)
            {
                var expectedCategory = categoriesListExample.FirstOrDefault(x => x.Id == cateogoryId);
                expectedCategory.Should().NotBeNull();
            }
        }
        ...
        public async Task Update()
        {
            ...
            genreCategoriesRelation.ForEach(genreCategoriesRelation =>
            {
                var expectedCategory = categoriesListExample.FirstOrDefault(x => x.Id == genreCategoriesRelation.CategoryId);
                expectedCategory.Should().NotBeNull();
            });
        }
        ...
        public async Task UpdateReplacingRelations()
        {
            ...
            await dbContext.Categories.AddRangeAsync(categoriesListExample, CancellationToken.None);
            await dbContext.Categories.AddRangeAsync(updateCategoriesListExample, CancellationToken.None);
            await dbContext.Genres.AddAsync(exampleGenre, CancellationToken.None);
            foreach (var categoryId in exampleGenre.Categories)
            {
                var relation = new GenresCategories(categoryId, exampleGenre.Id);
            ...
            genreCategoriesRelation.ForEach(genreCategoriesRelation =>
            {
                var expectedCategory = updateCategoriesListExample.FirstOrDefault(x => x.Id == genreCategoriesRelation.CategoryId);
                expectedCategory.Should().NotBeNull();
            });
        }
        ...
        public async Task SearchReturnsItemsAndTotal()
        {
            ...
            await dbContext.Genres.AddRangeAsync(exampleGenreList, CancellationToken.None);
            await dbContext.SaveChangesAsync(CancellationToken.None);
            var actDbContext = _fixture.CreateDbContext(true);
            var genreRepository = new Repository.GenreRepository(actDbContext);
            ...
            {
                var expectedGenre = exampleGenreList.FirstOrDefault(x => x.Id == genre.Id);
                expectedGenre.Should().NotBeNull();
                genre.Name.Should().Be(expectedGenre.Name);
                genre.IsActive.Should().Be(expectedGenre.IsActive);
                genre.CreatedAt.Should().Be(expectedGenre.CreatedAt);
            }
        }
        ...
        public async Task SearchReturnsRelations()
        {
            ...
            {
                var categoriesIdsToRelation = _fixture.GetExampleCategoriesList(random.Next(0, 4));
                if (categoriesIdsToRelation.Count > 0)
                {
                    categoriesIdsToRelation.ForEach(category => exampleGenre.AddCategory(category.Id));
                    dbContext.Categories.AddRange(categoriesIdsToRelation);
                    var relationsToAdd = categoriesIdsToRelation
            ...
            foreach (var genre in searchResult.Items)
            {
                ...
                genre.IsActive.Should().Be(expectedGenre.IsActive);
                genre.CreatedAt.Should().Be(expectedGenre.CreatedAt);
                genre.Categories.Should().BeEquivalentTo(expectedGenre.Categories);
            }
        }

        [Fact(DisplayName = nameof(SearchEmptyWhenPersistenceIsEmpty))]
        [Trait("Integration/Infra.Data", "GenreRepository - Repositories")]
        public async Task SearchEmptyWhenPersistenceIsEmpty()
        {
